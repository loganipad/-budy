...
export async function markEventProcessed(eventId, eventType, userId) {
  ...
  const existingPath = `/rest/v1/stripe_event_log?event_id=eq.${encodeURIComponent(id)}&select=event_id&limit=1`;
  const existing = await request(existingPath, { method: 'GET' });
  if (!existing.ok) return existing;

  if (Array.isArray(existing.data) && existing.data.length > 0) {
    return { ok: false, status: 409, duplicate: true, error: 'Event already processed.' };
  }
