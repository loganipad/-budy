...
export async function consumeDeepDiveCredit(input) {
  ...
  const rpcPayload = {
    ...
    p_email: input && input.email ? String(input.email).trim().toLowerCase() : null,
    p_credit_limit: creditLimit
  };

  const rpcResult = await request('/rest/v1/rpc/consume_ai_deep_dive_credit', {
    method: 'POST',
    body: JSON.stringify(rpcPayload)
  });

  if (rpcResult.ok) {
    const row = Array.isArray(rpcResult.data) ? rpcResult.data[0] || null : null;
    if (!row) {
      return { ok: false, error: 'AI Deep Dive credit operation returned no data.' };
    }

    const usedCount = Math.max(0, Number(row.used_count) || 0);
    const resolvedCreditLimit = Math.max(1, Number(row.credit_limit) || creditLimit);
    const remainingCredits = Math.max(0, Number(row.remaining_credits));
  ...
  const rpcMissing = rpcResult && typeof rpcResult.error === 'string' && /function\s+.*consume_ai_deep_dive_credit|does not exist/i.test(rpcResult.error);
  if (!rpcMissing) {
    return rpcResult;
  }
